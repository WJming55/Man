...
@interface TimeController ()<UITableViewDataSource,UITableViewDelegate>
...
-(void)createRightButtons{
    ...
    button1.frame = CGRectMake(0, 7, 30, 30);
    [button1 setBackgroundImage:[UIImage imageNamed:@"tag_b"] forState:UIControlStateNormal];
    [button1 addTarget:self action:@selector(goTag:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)goTag:(UIButton *)button{
    TagController *tvc = [[TagController alloc]init];
...
-(void)loadResourceWithPage:(NSInteger)page
{
    ...
    NSDictionary *dict = @{@"start":@(page),@"client":@(2),@"limit":@(10)};
    __weak typeof(self)weakSelf = self;
